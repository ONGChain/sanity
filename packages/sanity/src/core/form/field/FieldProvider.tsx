...
export function FieldProvider(
  ...
  const {hoveredStack} = useHoveredField()
  const hoveredPath = hoveredStack[hoveredStack.length - 1]
  const hovered =
    supportsTouch || (hoveredPath ? pathToString(path) === pathToString(hoveredPath) : false)
