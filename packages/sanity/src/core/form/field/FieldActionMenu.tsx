...
import {Card, Flex, Menu, MenuButton} from '@sanity/ui'
...
function renderAsButton(node: DocumentFieldActionNode) {
  return (node.type === 'menuItem' || node.type === 'group') && node.renderAsButton
}

/** @internal */
export function FieldActionMenu(props: FieldActionMenuProps) {
  const {nodes} = props
  const {focused, hovered} = useField()
  ...
  return (
    <Flex
      gap={1}
      style={{height: 25, lineHeight: 0, display: open || focused || hovered ? undefined : 'none'}}
    >
      {rootNodes.map((node, idx) => (
        <RootFieldActionMenuNode key={idx} node={node} onOpen={handleOpen} onClose={handleClose} />
      ))}
    </Flex>
  )
}

function RootFieldActionMenuNode(props: {
  node: DocumentFieldActionNode
  onOpen: () => void
  onClose: () => void
}) {
  const {node, onOpen, onClose} = props
  ...
  return <RootFieldActionMenuGroup node={node} onOpen={onOpen} onClose={onClose} />
}
